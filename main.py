...
if __name__ == "__main__":
    ...
    result = CurvedDNA(sequences * 10 , 'trifonov', name="DNAGeneratedwithPython")
    result.curvature[:, 18:22]
    result.save_csv('ArtificialDna.csv')
    result.save_pdb('ArtificialDna.pdb')
    result.plot('ArtificialDna.png', dpi=185)


    model = Sequential()
    model.add(Dense(3, input_shape=(sequences.encode().isdigit(),)))
